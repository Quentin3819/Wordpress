...
if ( ! class_exists('\WP_Smart_Image_Resize\Settings')) :
    class Settings
    {
        ...
        {
            if (_wp_sir_is_quota_exceeding_soon()) { ?>
...
/* #block-p */
        ...
        function settings_field_enable_trim()
        {
            ...
            <p class="description">
                ...
                if ( ! wp_sir_is_imagick_installed()) {
                    echo '<p class="wp-sir-notice-info"><span class="dashicons dashicons-info"></span> ';
                    _e('Enable Imagick PHP extension for faster processing (optional).', 'wp-smart-image-resize');
                    echo '</p>';
                }
                ?>
            </p>
...
/* #block-p */
        ...
        public function settings_field_enable($args)
        {
            ...
            /* block-f */
            ?>
            <p class="description" <?php echo _wp_sir_is_quota_exceeded()
                ? 'style="color:red"'
                : ''; ?>><?php echo _wp_sir_processed_quota(); ?> images of 150 processed. <a
        ...
        function settings_help($old_help, $screen_id, $screen)
        {
            ...
            $screen->add_help_tab(array(
                'id'      => 'wp-sir-help-tab2',
                'title'   => esc_html__('FAQ', WP_SIR_NAME),
                'content' =>
                    '<h3>How to resize existing images?</h3>' .
                    '<p>Install <a href="' .
                    admin_url(
                        'plugin-install.php?s=regenerate+thumbnails&tab=search&type=term'
                    ) .
                    '" target="_blank">Regenerate Thumbnails</a> plugin and follow the steps below: </p>' .
                    '<ol><li>Navigate to Tools > Regenerate Thumbnails</li>' .
                    '<li><p>Uncheck <b>Skip regenerating existing correctly sized thumbnails (faster).</b></p>' .
                    '<img src="' . (WP_SIR_URL . 'images/rt_setting1.png') .
                    '" alt="" width="400"></li>' .
                    '<li><p>Click on <b>Regenerate Thumbnails For All n Attachments</b> button.</p>' .
                    '<img src="' .
                    WP_SIR_URL .
                    'images/rt_setting2.png' .
                    '" alt="" width="400"></li></ol>'
            ));
